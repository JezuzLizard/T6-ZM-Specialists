...
register_perk()
{
	...
	perk_struct.hint_string = &"ZOMBIE_PERK_TOMBSTONE";
	perk_struct.perk_bottle = "zombie_perk_bottle_tombstone";
	perk_struct.perk_shader = "specialty_tombstone_zombies";
	...
	if ( isDefined( level.perk_precache_override_funcs[ "specialty_scavenger" ] ) )
	{
		perk_struct.precache_func = level.perk_precache_override_funcs[ "specialty_scavenger" ];
	}
	else 
	{
		perk_struct.precache_func = ::tombstone_precache;
	}
	perk_struct.clientfield_register = ::tombstone_register_clientfield;
	perk_struct.clientfield_set = ::tombstone_set_clientfield;
	perk_struct.funcs = [];
	if ( isDefined( level.perk_kvps_override_funcs[ "specialty_tombstone" ] ) )
	{
		perk_struct.perk_machine_set_kvps = level.perk_kvps_override_funcs[ "specialty_tombstone" ];
	}
	else 
	{
		perk_struct.perk_machine_set_kvps = ::tombstone_perk_machine_setup;
	}
	perk_struct.player_thread_give = ::tombstone_perk_give;
	perk_struct.player_thread_take = ::tombstone_perk_take;
	perk_struct.alias = "tombstone";
	maps/mp/zombies/_zm_perks::register_perk_basic_info( "specialty_scavenger", perk_struct );
}

tombstone_register_clientfield()
{
	registerclientfield( "toplayer", "perk_tombstone", 1, 2, "int" );
}

tombstone_set_clientfield( state )
{
	self setclientfieldtoplayer( "perk_tombstone", state );
}

tombstone_perk_machine_setup( use_trigger, perk_machine, bump_trigger, collision )
{
...
tombstone_precache()
{
	...
	level.machine_assets[ "specialty_scavenger" ] = spawnstruct();
	level.machine_assets[ "specialty_scavenger" ].off_model = "zombie_vending_tombstone";
	level.machine_assets[ "specialty_scavenger" ].on_model = "zombie_vending_tombstone_on";
}
...
init() //checked matches cerberus output
{
	...
	on_player_spawned_callback( ::set_player_tombstone_index );
