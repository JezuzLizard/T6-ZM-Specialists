...
register_perk()
{
	...
	perk_struct.hint_string = &"ZOMBIE_PERK_CHUGABUD";
	perk_struct.perk_bottle = "zombie_perk_bottle_whoswho";
	perk_struct.perk_shader = "specialty_chugabud_zombies";
	...
	if ( isDefined( level.perk_precache_override_funcs[ "specialty_finalstand" ] ) )
	{
		perk_struct.precache_func = level.perk_precache_override_funcs[ "specialty_finalstand" ];
	}
	else 
	{
		perk_struct.precache_func = ::chugabud_precache;
	}
	perk_struct.clientfield_register = ::chugabud_register_clientfield;
	perk_struct.clientfield_set = ::chugabud_set_clientfield;
	perk_struct.funcs = [];
	if ( isDefined( level.perk_kvps_override_funcs[ "specialty_finalstand" ] ) )
	{
		perk_struct.perk_machine_set_kvps = level.perk_kvps_override_funcs[ "specialty_finalstand" ];
	}
	else 
	{
		perk_struct.perk_machine_set_kvps = ::chugabud_perk_machine_setup;
	}
	perk_struct.player_thread_give = ::chugabud_perk_take;
	perk_struct.player_thread_take = ::chugabud_perk_give;
	perk_struct.alias = "chugabud";
	maps/mp/zombies/_zm_perks::register_perk_basic_info( "specialty_finalstand", perk_struct );
}

chugabud_register_clientfield()
{
	registerclientfield( "toplayer", "perk_chugabud", 1000, 2, "int" );
}

chugabud_set_clientfield( state )
{
	self setclientfieldtoplayer( "perk_chugabud", state );
}

chugabud_perk_machine_setup( use_trigger, perk_machine, bump_trigger, collision )
{
...
chugabud_precache()
{
    ...
    level.machine_assets[ "specialty_finalstand" ] = spawnstruct();
    level.machine_assets[ "specialty_finalstand" ].off_model = "p6_zm_vending_chugabud";
    level.machine_assets[ "specialty_finalstand" ].on_model = "p6_zm_vending_chugabud_on";
}
